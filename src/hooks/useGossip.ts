...
function applyLocally(gossipMessage: GossipMessage, connect: (peerId: string) => void, peerId: string | null): boolean {
...
export function useGossip(
	...
	const receive = useCallback(
		(gossipMessage: GossipMessage, senderId: string) => {
			if (seenMessageIds.has(gossipMessage.id)) return;
			seenMessageIds.add(gossipMessage.id);

			const shouldRebroadcast = applyLocally(gossipMessage, connect, peerId);
