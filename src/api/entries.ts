...
export const getEntries = async (): Promise<Entry[]> => {
	...
	return result.map(({ songPackage, c3Comments, o, l, a, k }, index): Entry => {
		...
		return {
			...
			vocalsRank: songPackage.rank.vocals ?? 0,
			guitarRank: songPackage.rank.guitar ?? 0,
			drumRank: songPackage.rank.drum ?? 0,
			bassRank: songPackage.rank.bass ?? 0,
			keysRank: songPackage.rank.keys ?? 0,
			proKeysRank: songPackage.rank.real_keys ?? 0,
			proGuitarRank: songPackage.rank.real_guitar ?? 0,
			proBassRank: songPackage.rank.real_bass ?? 0,
			bandRank: songPackage.rank.bank ?? 0,
